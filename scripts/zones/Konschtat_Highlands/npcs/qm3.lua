...
local konschtatGlobal = require("scripts/zones/Konschtat_Highlands/globals")
...
entity.onSpawn = function(npc)
    npc:timer(popmovetimer, function()
        konschtatGlobal.moveGoblinArchaeologistQM()
    end)
end
...
entity.onTrade = function(player, npc, trade)
    ...
    if (npc:getLocalVar('KHGADEPTH')) and (npc:getLocalVar('KHGADEPTH') > 0) then
        currentDepth = npc:getLocalVar('KHGADEPTH')
    end
    ...
    if currentDepth > 0 then
        ...
        npc:setLocalVar('KHGADEPTH',currentDepth)
    else
